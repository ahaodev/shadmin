...
func TestApplyRolePolicyDiff_NeverRevokesUnchanged(t *testing.T) {
	...
	for i, snap := range sm.snapshots {
		if !slices.Contains(snap, policyRule{obj: "/b", act: "GET"}) ||
			!slices.Contains(snap, policyRule{obj: "/c", act: "GET"}) {
...
func TestApplyUserRoleDiff_NeverRevokesUnchanged(t *testing.T) {
	...
	for i, snap := range sm.snapshots {
		if !slices.Contains(snap, "r2") || !slices.Contains(snap, "r3") {
...
// 且目标集为空时必须撤销该角色的全部策略。
func TestApplyRolePolicyDiff_RevokesAllAndLeavesOtherRolesUntouched(t *testing.T) {
	base := testManager(t)
	mustAddPolicy(t, base, "role-1", "/a", "GET")
	mustAddPolicy(t, base, "role-1", "/a2", "DELETE")
	mustAddPolicy(t, base, "role-2", "/b", "POST")
	mustAddPolicy(t, base, "role-2", "/c", "POST")

	svc := &SyncService{manager: base}
	// role-1 收敛到空集：应撤销 role-1 的全部策略。
