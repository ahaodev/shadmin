...
// 仅在用户主动登出时写入；过期后由底层 cachex.Cacher 自动清理。
// 缓存后端（进程内存或 Redis）由注入的 cachex.Cacher 决定，本包不再自行实现双后端。
...
// New 返回基于 cachex.Cacher 的黑名单实现。
// 内存/Redis 的后端选择由调用方通过 cacher 一次性决定。
func New(cacher cachex.Cacher) Blacklist {
