...
func initMenu(app *Application, roleID string) {
	...
	menu.Create().SetParentID(deptMgmt.ID).SetName("创建部门").SetSequence(0).SetType("button").SetPath("").SetIcon("PlusCircle").SetIsFrame(false).SetVisible("show").SetPermissions("system:dept:add").AddAPIResourceIDs("POST:/api/v1/system/department", "GET:/api/v1/system/department/tree").SetStatus(constants.StatusActive).Save(ctx)
	menu.Create().SetParentID(deptMgmt.ID).SetName("编辑部门").SetSequence(0).SetType("button").SetPath("").SetIcon("Edit").SetIsFrame(false).SetVisible("show").SetPermissions("system:dept:edit").AddAPIResourceIDs("PUT:/api/v1/system/department/:id", "GET:/api/v1/system/department/:id", "GET:/api/v1/system/department/tree").SetStatus(constants.StatusActive).Save(ctx)
	menu.Create().SetParentID(deptMgmt.ID).SetName("删除部门").SetSequence(0).SetType("button").SetPath("").SetIcon("Trash2").SetIsFrame(false).SetVisible("show").SetPermissions("system:dept:delete").AddAPIResourceIDs("DELETE:/api/v1/system/department/:id").SetStatus(constants.StatusActive).Save(ctx)
	// Log button
	menu.Create().SetParentID(loginLogs.ID).SetName("清空日志").SetSequence(0).SetType("button").SetPath("").SetIcon("Trash2").SetIsFrame(false).SetVisible("show").SetPermissions("system:login_logs:clean").AddAPIResourceIDs("DELETE:/api/v1/system/login-logs").SetStatus(constants.StatusActive).Save(ctx)
