...
func initMenu(app *Application, roleID string) {
	...
	apiMapping, _ := app.DB.Menu.Create().SetParentID(system.ID).SetName("API资源").SetSequence(4).SetType("menu").SetPath("/system/api-resources").SetIcon("Code2").SetIsFrame(false).SetVisible("show").AddAPIResourceIDs("GET:/api/v1/system/api-resources").SetStatus("active").Save(ctx)
	loginLogs, _ := app.DB.Menu.Create().SetParentID(system.ID).SetName("登录日志").SetSequence(5).SetType("menu").SetPath("/system/login-logs").SetIcon("Layers3").SetIsFrame(false).SetVisible("show").AddAPIResourceIDs("GET:/api/v1/system/login-logs").SetStatus("active").Save(ctx)
	dictMgmt, _ := app.DB.Menu.Create().SetParentID(system.ID).SetName("字典管理").SetSequence(6).SetType("menu").SetPath("/system/dict").SetIcon("BookMarked").SetIsFrame(false).SetVisible("show").AddAPIResourceIDs("GET:/api/v1/system/dict/types", "GET:/api/v1/system/dict/items", "GET:/api/v1/system/dict/types/code/:code/items").SetStatus("active").Save(ctx)
